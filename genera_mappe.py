...
print("=== GENERAZIONE MAPPA LAZIO STILE METEO CLOUD ===")
...
# 1. Griglia più ampia della visuale: copre un'area più grande per evitare qualsiasi taglio sui bordi
lats = np.linspace(40.2, 43.5, 35)
lons = np.linspace(10.8, 14.8, 35)
...
print("Scaricamento dati griglia estesa...")
with concurrent.futures.ThreadPoolExecutor(max_workers=25) as executor:
    results = executor.map(fetch_point, tasks)
    for i, j, val in results:
        Data_Grid[i, j] = val

# Livelli termici fluidi
levels = np.arange(-10, 44, 1)
cmap = plt.get_cmap('Spectral_r')

# Stile visivo scuro identico a Meteo Cloud
fig = plt.figure(figsize=(10, 9), facecolor='#1a1a1a')
ax = plt.axes(projection=ccrs.PlateCarree())
ax.set_facecolor('#1a1a1a')

# 2. Finestra visiva centrata esattamente sul Lazio
ax.set_extent([11.2, 14.2, 41.0, 42.8], crs=ccrs.PlateCarree())

# Disegno delle sfumature termiche che riempiono tutto lo spazio
mesh = ax.contourf(Lon, Lat, Data_Grid, transform=ccrs.PlateCarree(), cmap=cmap, levels=levels, extend='both', alpha=0.95, zorder=1)

# Elementi geografici con colori abbinati allo stile scuro
...
# Barra dei colori in basso stile professionale
...
print("Mappa generata con successo in stile Meteo Cloud!")
